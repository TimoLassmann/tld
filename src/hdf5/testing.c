...
tld_internal int hdf5_open_group(hdf5_data* hdf5_data, char* groupname);
tld_internal int hdf5_close_group(hdf5_data* hdf5_data);

tld_internal int hdf5_data_alloc(hdf5_data **h);
tld_internal void hdf5_data_free(hdf5_data* h);

int hdf5_prepare_read(struct hdf5_data *hdf5_data, char *group, char *name)
{
        RUN(hdf5_open_group(hdf5_data, group));
...
int hdf5_finalise_read(struct hdf5_data *hdf5_data)
{
...
int tld_h5D_read_wrap(struct hdf5_data *hdf5_data)
{
        hdf5_data->status = H5Dread(hdf5_data->dataset, hdf5_data->datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,hdf5_data->data);
        return OK;
}

int hdf5_compare_data_types(struct hdf5_data *hdf5_data)
{
...
int hdf5_write(struct hdf5_data *hdf5_data, char *group, char *name, void *data)
{
        RUN(hdf5_open_group(hdf5_data, group));
        snprintf(hdf5_data->dataset_name , MAX_NAME_LEN,"%s",name);
        ...
        if(!hdf5_data->status){
                ...
                if(!H5Tequal(hdf5_data->datatype, hdf5_data->native_type)){
                        WARNING_MSG("Writing into an existing dataset filed");
                ...
                /* Make sure that existing dataspace has the correct dimentions */
...
int tldhdf5_open_file(hdf5_data** h, char* filename)
{
        hdf5_data* hdf5_data = NULL;
...
int tlfhdf5_close_file(struct hdf5_data** h)
{
...
int hdf5_open_group(hdf5_data* hdf5_data, char* groupname)
{
        ...
        l = strnlen(groupname, MAX_NAME_LEN);
        ...
        for(i = 1; i < l;i++){
                if(groupname[i] == '/'){
                        ...
                        if (hdf5_data->status == 0){
                                snprintf(hdf5_data->group_name , MAX_NAME_LEN,"%s",buffer);
...
int hdf5_close_group(hdf5_data* hdf5_data)
{
...
int hdf5_data_alloc(hdf5_data** h)
{
        int i;
        hdf5_data* hdf5_data = NULL;
...
void hdf5_data_free(hdf5_data* hdf5_data)
{
        if(hdf5_data){
...
/*                 hdf5_data->data = NULL;                                 \ */
/*                                                                         \ */
/*                 galloc(&p, hdf5_data->dim[0],hdf5_data->dim[1]);    \ */
/*                 HDFWRAP_START_GALLOC(p,&ptr);                           \ */
/*                 hdf5_data->status = H5Dread(hdf5_data->dataset, hdf5_data->datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,ptr); \ */
/*                                                                         \ */
/*                 if((hdf5_data->status = H5Tclose(hdf5_data->datatype)) < 0) ERROR_MSG("H5Tclose failed"); \ */
