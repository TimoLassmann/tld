...
int tld_thread_pool_add(tld_thread_pool *p, void (*func_ptr)(void *), void *data)
{
        LOG_MSG("Adding work");
        pthread_mutex_lock(&p->lock);
...
void *pool_worker(void *data)
{
        ...
        while(1){
                ...
                if (!p->stop && p->n_working == 0 && !p->n_work){
...
int tld_thread_pool_alloc(tld_thread_pool **pool, int n_threads)
{
...
void  tld_thread_pool_wait(tld_thread_pool *p)
{
        if(p){
                pthread_mutex_lock(&p->lock);
                while (1) {
                        LOG_MSG("Waiting: %d threads %d work %d running", p->n_threads, p->n_work, p->n_working);
                        if ((!p->stop && p->n_working != 0) || (p->stop && p->n_threads != 0)) {
