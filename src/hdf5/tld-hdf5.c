...
#define ADD_ARRAY(type)                                                 \
        int tld_hdf5_add_0D_dataset_ ## type(struct hdf5_data* hdf5_data, char* group, char* name, type data) \
        {                                                               \
                TLD_HDF5_ADD_SINGLE_BODY;                               \
        }
...
#define ADD_ARRAY(type)                                                 \
        int tld_hdf5_add_1D_dataset_ ##type(struct hdf5_data* hdf5_data, char* group, char* name, type* data) \
        {                                                               \
                TLD_HDF5_ADD_BODY;                                      \
        }\
        int tld_hdf5_add_2D_dataset_ ##type(struct hdf5_data* hdf5_data, char* group, char* name, type** data) \
        {                                                               \
                TLD_HDF5_ADD_BODY;                                      \
        }
...
#define READ_ARRAY(type)                                                \
        int tld_hdf5_read_0D_dataset_ ##type(struct hdf5_data* hdf5_data, char* group, char* name, type* data) \
        {                                                               \
                RUN(tld_hdf5_open_group(hdf5_data, group));             \
...
#define READ_ARRAY(type)                                                \
        int tld_hdf5_read_2D_dataset_ ##type(struct hdf5_data* hdf5_data, char* group, char* name, type*** data) \
        {                                                               \
                void* ptr = NULL;                                       \
                type** p = NULL;                                          \
                RUN(tld_hdf5_open_group(hdf5_data, group));             \
...
#define ADD_ATTR(type)                                                  \
        int tld_hdf5_add_attribute_ ##type(struct hdf5_data* hdf5_data, char* group, char* name,type x) \
        {                                                               \
                hid_t aid;                                              \
                hid_t attr;                                             \
                RUN(tld_hdf5_open_group(hdf5_data, group));             \
...
int tld_hdf5_add_attribute_string(struct hdf5_data* hdf5_data,char* group, char* name,char* x)
{
        hid_t aid;
        hid_t atype;
        hid_t attr;
        ...
        len = strnlen(x, TLD_HDF5_MAX_CONTENT_LEN) + 1;
...
#define READ_ATTR(type)                                                 \
        ...
        {                                                               \
                ...
                RUN(tld_hdf5_open_group(hdf5_data, group));             \
                RUN(my_H5Oget_info(hdf5_data->group, &oinfo));          \
...
int tld_hdf5_read_attribute_string(struct hdf5_data* hdf5_data, char* group,char* name, char** x)
{
        H5O_info_t oinfo;
...
int tld_hdf5_open_group(struct hdf5_data* hdf5_data, char* groupname)
{
        ...
        l = strnlen(groupname, TLD_HDF5_MAX_NAME_LEN);
        ...
        for(i = 1; i < l;i++){
                if(groupname[i] == '/'){
                        ...
                        if (hdf5_data->status == 0){
                                snprintf(hdf5_data->group_name , TLD_HDF5_MAX_NAME_LEN,"%s",buffer);
                                ...
                                RUN(tld_hdf5_close_group(hdf5_data));
                        }
                }
        }
        /* hdf5_data->status = H5Eset_auto(hdf5_data->status ,NULL, NULL); */
        ...
        if (hdf5_data->status == 0){
                ...
                hdf5_data->group = H5Gcreate (hdf5_data->file , hdf5_data->group_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                if(hdf5_data->group < 0){
                        ERROR_MSG("H5Gcreate failed\n");
                }
                /* if((hdf5_data->group = H5Gcreate (hdf5_data->file , hdf5_data->group_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)ERROR_MSG("H5Gcreate failed\n"); */
                RUN(tld_hdf5_close_group(hdf5_data));
        }
        ...
        hdf5_data->group = H5Gopen(hdf5_data->file,  groupname , H5P_DEFAULT);
        if(hdf5_data->group < 0){
                ERROR_MSG("H5Gopen failed\n");
        }

        /* LOG_MSG("Open group dine"); */
...
int tld_hdf5_close_group(struct hdf5_data* hdf5_data)
{
        hdf5_data->status = H5Gclose(hdf5_data->group);

        if(hdf5_data->status < 0){
                ERROR_MSG("H5Gclose failed");
        }
...
int tld_hdf5_open_file(struct hdf5_data** h, char* filename)
{
...
int tld_hdf5_close_file(struct hdf5_data** h)
{
        struct hdf5_data* hdf5_data = NULL;
...
herr_t op_func (hid_t loc_id, const char *name, const H5L_info_t *info, void *operator_data)
{
        ...
        int len_t,len_q;
        UNUSED(info);
        RUN(my_H5Oget_info_by_name(loc_id, name, &infobuf));
/* #if defined(H5Oget_info_vers) && H5Oget_info_vers == 3 */
/*         LOG_MSG("Version 3"); */
...
/* #endif  */
        switch (infobuf.type){
        case H5O_TYPE_GROUP:
                len_t = strnlen(name, TLD_HDF5_MAX_NAME_LEN);
                len_q = strnlen( look[0], TLD_HDF5_MAX_NAME_LEN);
                if(len_t >= len_q){ /* match is possible */
                        if(!strncmp(name + (len_t - len_q), look[0], TLD_HDF5_MAX_NAME_LEN)){
                                if(look[1]){
...
int my_H5Oget_info(hid_t group, H5O_info_t *info)
{
        int status;
        //H5O_info_t oinfo;
#if defined(H5Oget_info_vers) && H5Oget_info_vers == 3
        status = H5Oget_info(group , info,H5O_INFO_NUM_ATTRS);
#else
        status = H5Oget_info(group , info);
#endif
        if(status < 0){
                ERROR_MSG(" H5Oget_info_by_name failed. %d" , status);
        }
